...
# todo: replace this with smart check on "docker-compose" vs "docker compose"
...
if $stop; then
  services="${services} grafana quepid keycloak"
...
# curl -u 'admin:admin' -X POST "localhost:9200/_security/user/chorus_admin?pretty" -H 'Content-Type: application/json' -d'
# {
#   "password" : "password",
#   "roles" : ["superuser"]
# }
...
# curl -u 'admin:admin' -X POST "localhost:9200/_security/role/anonymous_user" -H 'Content-Type: application/json' -d'
# {
#   "run_as": [ ],
#   "cluster": [ ],
#   "indices": [
...
curl -u 'admin:admin' -s -X PUT "localhost:9200/ecommerce/" -H 'Content-Type: application/json' --data-binary @./opensearch/schema.json
...
if $vector_search; then
  ...
  curl -u 'admin:admin' -s -X POST "localhost:9200/ecommerce/_bulk?pretty" -H 'Content-Type: application/json' --data-binary @transformed_data.json
...
# curl -u 'admin:admin' -s --request PUT 'http://localhost:9200/_querqy/rewriter/common_rules' \
...
# curl -u 'admin:admin' -s --request PUT 'http://localhost:9200/_querqy/rewriter/common_rules_prelive' \
# --header 'Content-Type: application/json' \
# --data-raw '{
#     "class": "querqy.opensearch.rewriter.SimpleCommonRulesRewriterFactory",
#     "config": {
...
# curl -u 'admin:admin' -s --request PUT 'http://localhost:9200/_querqy/rewriter/replace' \
# --header 'Content-Type: application/json' \
# --data-raw '{
...
# curl -u 'admin:admin' -s --request PUT 'http://localhost:9200/_querqy/rewriter/replace_prelive' \
# --header 'Content-Type: application/json' \
# --data-raw '{
#     "class": "querqy.opensearch.rewriter.ReplaceRewriterFactory",
#     "config": {
...
#   curl -u 'admin:admin' -s --request PUT 'http://localhost:9200/_querqy/rewriter/embtxt' \
#   --header 'Content-Type: application/json' \
#   --data-raw '{
...
#   curl -u 'admin:admin' -s --request PUT 'http://localhost:9200/_querqy/rewriter/embimg' \
#   --header 'Content-Type: application/json' \
#   --data-raw '{
#       "class": "querqy.opensearch.rewriter.EmbeddingsRewriterFactory",
#       "config": {
...
if $offline_lab; then
  ...
  ${COMPOSE_CMD} run --rm quepid bin/rake db:setup
  ${COMPOSE_CMD} run quepid thor user:create -a admin@choruselectronics.com "Chorus Admin" password
fi
