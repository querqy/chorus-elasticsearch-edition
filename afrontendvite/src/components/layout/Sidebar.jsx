    ...
    <div className="flex flex-col gap-6 w-full pr-2 select-none">
      ...
      {selectedAlgo === "other_config" && (
        <div className="flex flex-col gap-2 pl-2 border-l-2 border-blue-500 animate-fadeIn">
          <h4 className="text-xs font-semibold text-gray-600">Select Configuration:</h4>
          <select
          ...
          >
            ...
            <option value="baseline_title_weight">baseline with title weight</option>
      ...
      <div className="flex flex-col gap-2">
        <h3 className="font-bold text-sm text-gray-800">Filter by Product Types</h3>
            ...
            typeAggs.map((bucket) => (
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  />
                  <span className="truncate max-w-[160px]" title={bucket.key}>{bucket.key}</span>
                </div>
                <span className="text-gray-400 text-right pr-1 text-xs">{bucket.doc_count}</span>
              </label>
            ))
          ) : (
            <div className="text-sm text-gray-500 italic">No filters available</div>
          )}
        </div>
      </div>
    </div>
