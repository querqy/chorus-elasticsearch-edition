...
export default function ProductCard({ product }) {
  return (
    <Link to={`/product/${product.id}`} className=" border border-gray-200 rounded p-4 flex flex-col items-center hover:shadow-lg transition-shadow bg-white h-full cursor-pointer text-inherit no-underline">
      <img
      ...
      <div className="mt-auto w-full flex items-center justify-center gap-2 border-t pt-3">
        <button onClick={(e) => e.preventDefault()} className="flex items-center gap-1 text-sm text-gray-700 hover:text-blue-600">
          <span>Add to</span>
