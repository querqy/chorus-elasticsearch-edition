  ...
  return (
    <div className="h-screen flex flex-col bg-white text-gray-900 font-sans overflow-y-auto">
      <header className="flex items-start p-6 border-b border-gray-200">
        <div className="w-72 flex-shrink-0 flex justify-center cursor-pointer" onClick={() => navigate('/')}>
      ...
      <main className="p-10 max-w-5xl mx-auto w-full flex flex-col md:flex-row gap-10">
        ...
        <div className="w-full md:w-1/2 flex flex-col gap-4">
          <button onClick={() => navigate(-1)} className="text-blue-600 hover:underline w-fit mb-2 font-semibold">
            &larr; Back to Results
          </button>
          
          <h1 className="text-3xl font-bold text-gray-800" dangerouslySetInnerHTML={{ __html: product.title }} />
          <p className="text-sm text-gray-500">Supplier: {product.supplier} | EAN: {product.ean?.[0]}</p>
          ...
          <div className="bg-gray-50 p-4 rounded border border-gray-100 mt-4">
            ...
            <p className="text-gray-700 leading-relaxed">{product.short_description || 'No description available.'}</p>
          </div>
        </div>
      </main>
    </div>
